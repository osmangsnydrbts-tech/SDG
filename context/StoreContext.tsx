...
interface StoreData {
  ...
  collectSalesToTreasury: (companyId: number, employeeId: number, amount: number) => Promise<{ success: boolean; message: string }>;
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const performSale = async (employeeId: number, companyId: number, productName: string, amount: number) => {
      ...
      // Add to Employee's Sales Balance (Not Main)
      const empTreasury = treasuries.find(t => t.employee_id === employeeId);
      if (!empTreasury) return { success: false, message: 'خزينة الموظف غير موجودة' };
      ...
      try {
        if (transaction.type === 'exchange') {
            ...
            if (transaction.from_currency === 'SDG') {
             ...
             // Reversal: Deduct from EMPLOYEE Treasury Sales Balance
             if (!empTreasury) throw new Error('خزينة الموظف غير موجودة');
  ...
  const collectSalesToTreasury = async (companyId: number, employeeId: number, amount: number) => {
    // Ensure integer
    amount = Math.round(amount);
    ...
    const currentSalesBalance = empTreasury.sales_balance || 0;
    if (currentSalesBalance < amount) return { success: false, message: 'رصيد مبيعات الموظف غير كافي' };

    // Deduct from Employee Sales Balance
    await supabase.from('treasuries').update({
        sales_balance: Math.round(currentSalesBalance - amount)
    }).eq('id', empTreasury.id);
    
    // Add to Main Treasury EGP Balance
    await supabase.from('treasuries').update({
        egp_balance: Math.round(mainTreasury.egp_balance + amount)
    }).eq('id', mainTreasury.id);

    const employeeName = users.find(u => u.id === employeeId)?.full_name || 'موظف';
    ...
    await supabase.from('transactions').insert({
        ...
        from_amount: amount,
        from_currency: 'EGP',
        description: `ترحيل مبيعات من ${employeeName}`,
        created_at: new Date().toISOString()
    });
